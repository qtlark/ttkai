...
function ReplyMessage(props: ReplyMessageProps) {
    ...
    if (imgRegex.test(jscontent.orignmsg)) {
        ...
        content = `<font color=8A2BE2>${jhconvert(jscontent.replywho)}</font><div style="text-align: center;"><img src=${jscontent.orignmsg} style="width: ${p_width}px; height: ${p_height}px"></div><hr>${jhconvert(jscontent.replymsg)}`;
        return (
            <div
